...
BEGIN
  ...
  -- etapas informadas nos itens precisam ser da MESMA empresa
  IF EXISTS (
    SELECT 1 FROM jsonb_array_elements(COALESCE(_itens,'[]'::jsonb)) it
     WHERE NULLIF(it->>'etapa_id','') IS NOT NULL
       AND NOT EXISTS (
         SELECT 1 FROM public.obra_etapas e
          WHERE e.id = (it->>'etapa_id')::UUID AND e.empresa_id = v_empresa)
  ) THEN
    RAISE EXCEPTION 'Etapa informada não pertence a esta empresa';
  END IF;

  DELETE FROM public.orcamento_itens WHERE orcamento_id = v_id AND empresa_id = v_empresa;

  INSERT INTO public.orcamento_itens (
    orcamento_id, empresa_id, servico_id, codigo, tipo, descricao, descricao_detalhada,
    unidade, quantidade, preco_unitario, desconto_pct, aliquota_iss, bdi_pct, etapa_id, ordem
  )
  SELECT v_id, v_empresa,
    ...
    NULLIF(it->>'bdi_pct','')::NUMERIC,
    NULLIF(it->>'etapa_id','')::UUID,
